...
mixin DecorationPainter on ShapeBorder {
  ...
  void paintDecoration(Canvas canvas, Rect rect, {TextDirection? textDirection}) {
    ...
    if (innerShadow.isNotEmpty) {
      ...
      for (final BoxShadow boxShadow in innerShadow) {
        final Paint paint = boxShadow is GradientShadow ? boxShadow.toPaintRect(rect, textDirection: textDirection) : boxShadow.toPaint();
        final Rect bounds = rect.shift(boxShadow.offset).deflate(boxShadow.spreadRadius);
        final outerPath = getOuterPath(bounds, textDirection: textDirection);
        outerPath.addRect(bounds.inflate(boxShadow.blurRadius + boxShadow.spreadRadius + max(boxShadow.offset.dx, boxShadow.offset.dy)));
    ...
    if (shadow.isNotEmpty) {
      ...
      if (clipInner) {
        ...
        for (final BoxShadow boxShadow in shadow) {
          final curSpreadDistane = (boxShadow.blurRadius + boxShadow.spreadRadius + max(boxShadow.offset.dx, boxShadow.offset.dy)) * 2;
      ...
      for (final BoxShadow boxShadow in shadow) {
        ...
        canvas.drawPath(getOuterPath(bounds, textDirection: textDirection), paint);
      }
    }
  }

  void paintBorder2(Canvas canvas, Rect rect, GradientBorderSide side, {TextDirection? textDirection}) {
    final innerPath = getInnerPath(rect, textDirection: textDirection);
    final outerPath = getOuterPath(rect, textDirection: textDirection);
